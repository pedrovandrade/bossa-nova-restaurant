...
const DEFAULT_DAY = { isOpen: false, timespans: [{ begin: '10:00', end: '12:00' }] };
...
const OpeningHoursPage: FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const openingHoursData: OpeningHoursData = await res.json();
        if (mounted) setOpeningHours(openingHoursData);
      } catch (err) {
        console.error(err);
  ...
  function addTimespan(weekday: Weekday) {
    ...
    setOpeningHours((prev) => {
      ...
      const prevEnd = last ? last.end : '10:00';
