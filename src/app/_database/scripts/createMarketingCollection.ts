...
import { marketingData } from '@/database/seed/marketingSeed';
...
async function run() {
  ...
  try {
    ...
    const Marketing = mongoose.models.Marketing || mongoose.model('Marketing', MarketingSchema, COLL_NAME);

    // expect seed export to be an object representing the single marketing document
    const doc = marketingData ?? {};

    const res = await Marketing.insertOne({ ...doc });
    console.log(`Upserted marketing document with _id="${res._id}". Result:`, res);
  } catch (err) {
    console.error('Error creating/seeding marketing collection:', err);
    process.exitCode = 1;
  } finally {
